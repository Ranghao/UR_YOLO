...
class SCRConv(nn.Module):
    default_act = nn.SiLU()  # default activation
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, dilation=1):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
        self.SCRConv = ScrConv(c2)
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()

    def forward(self, x):
        return self.act(self.bn(self.SCRConv(self.conv(x))))

    def forward_fuse(self, x):
        return self.act(self.SCRConv(self.conv(x)))
...
class CRU(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.TOPK_channel = up_channel = int(alpha * op_channel)
        self.others_channel = low_channel = op_channel - up_channel
        ...
        self.fc = nn.Linear(op_channel, 1)

    def forward(self, x):
        b, c, _, _ = x.size()
        x_avg = F.adaptive_avg_pool2d(x, 1).view(b, c)  # Global average pooling
        weights = torch.sigmoid(self.fc(x_avg)).view(b, 1, 1, 1)  # Learnable weights
        weighted_x = x * weights

        # Split
        up, low = torch.split(weighted_x, [self.TOPK_channel, self.others_channel], dim=1)
...
class SPPELANBRA(nn.Module):
    ...
    def forward(self, x):
        y = [self.cv1(x)]
        y.extend(m(y[-1]) for m in [self.cv2, self.cv3, self.cv4])
        return self.att(self.cv5(torch.cat(y, 1)))
